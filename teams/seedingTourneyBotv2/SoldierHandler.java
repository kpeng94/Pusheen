		...
		if (rc.readBroadcast(30000) != id) {
			...
			if (targetLocation.x != -1 && (rc.getLocation().x == targetLocation.x &&
				...
				if (dir == Direction.EAST) {
					...
					rc.broadcast(15001 + pastrLoc.y - targetLocation.y + 2, Clock.getRoundNum());
				} else if (dir == Direction.WEST) {
					rc.broadcast(20000 + targetLocation.y - pastrLoc.y + 2, id);
					rc.broadcast(15001 + targetLocation.y - pastrLoc.y + 2, Clock.getRoundNum());
				} else if (dir == Direction.NORTH) {
					rc.broadcast(20000 + pastrLoc.x - targetLocation.x + 2, id);
					rc.broadcast(15001 + pastrLoc.x - targetLocation.x + 2, Clock.getRoundNum());
				} else if (dir == Direction.SOUTH) {
					rc.broadcast(20000 + targetLocation.x - pastrLoc.x + 2, id);
					rc.broadcast(15001 + targetLocation.x - pastrLoc.x + 2, Clock.getRoundNum());
				}
			}			
		}
	}
